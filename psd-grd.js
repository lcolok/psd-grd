...
(function (PSDGradient) {
  var CSSCreator = (function () {
    ...
    CSSCreator.prototype.createGradientStyle = function (grad, options) {
      ...
      var stops = this.createGradeientStops(grad)
        .map(function (e) {
          return "    " + e;
        })
        .join(",\n");
    ...
    CSSCreator.prototype.create = function (grad, options) {
      ...
      var css = [selector, this.createGradientStyle(grad, options), "}"].join(
        "\n"
      );
      return css;
    };
    CSSCreator.prototype.createGradeientStops = function (grad_obj) {
      return grad_obj.gradient_stops.map(function (grad) {
        ...
        if (grad.color_stop.color_obj.type == "RGBC") {
          ...
          return "rgba(#rgba) #lctn%"
            .replace("#rgba", rgba)
            .replace("#lctn", lctn.toString());
        } else {
          var hsla = [
...
(function (PSDGradient) {
  var SVGCreator = (function () {
    function SVGCreator() {}
    SVGCreator.prototype.createZpaceGradientStyle = function (grad, options) {
      console.log(grad);
      ...
      var stops = JSON.stringify(this.createZpaceGradeientStops(grad));
      // .map(function (e) {
      //     return "    " + e;
      // }).join("\n");
      return stops;
      ...
      return grad_obj.gradient_stops
        .map(function (grad) {
          ...
          if (grad.color_stop.color_obj.type == "RGBC") {
            ...
            return { rgba, lctn };
            return "<stop stop-color='rgba(#rgba)' offset='#lctn%'/>"
              .replace("#rgba", rgba)
              .replace("#lctn", lctn.toString());
          } else {
            var hsla = [
    ...
    SVGCreator.prototype.createGradeientStops = function (grad_obj) {
      return grad_obj.gradient_stops.map(function (grad) {
        ...
        if (grad.color_stop.color_obj.type == "RGBC") {
          ...
          return "<stop stop-color='hsla(#hsla)' offset='#lctn%'/>"
            .replace("#hsla", hsla)
            .replace("#lctn", lctn.toString());
        }
      });
              ...
              if (obj_type == "ObjcT") {
                ...
                if (obj_type == "VlLsT") {
                  ...
                  if (obj_type == "Discriptor") {
                    ...
                    throw new Error(
                      "SimpleObjectCreator not support type:" + obj_type
                    );
                  }
                }
              }
